...
class IsRecepcionistOrAdm(BasePermission):
...
class LoginPatientUser(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if patient_serializer.data["last_login"] == None and "new_password" not in request.data :
            return Response({"detail": "New patients need to send a key new_password to update their password before login"}, status.HTTP_400_BAD_REQUEST)
